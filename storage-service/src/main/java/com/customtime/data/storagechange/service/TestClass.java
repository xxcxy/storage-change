...
//	        new InputStreamReader(TestClass.class.getResourceAsStream("/client_secrets.json")));
...
//	  }
	  ...
	  private static HttpRequestInitializer getHRI(){
		  return new ClientParametersAuthentication("clientId","clientSecret");
	  }

	  /** Loads sample-specific settings from a JSON file. */
	  private static void readSettings() {
	    settings = SampleSettings.load(JSON_FACTORY,TestClass.class.getResourceAsStream("/sample_settings.json"));
...
//	        Credential credential = authorize();
	        ...
	        storage = new Storage.Builder(httpTransport, JSON_FACTORY, getHRI()).setApplicationName(APPLICATION_NAME).build();
	        // run commands
	        tryCreateBucket();
	        getBucket();
	        listObjects();
